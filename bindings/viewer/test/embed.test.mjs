...
// once with the kernel on the page and once with the kernel in the worker.
...
const check = (condition, message) => {
  console.log(`${condition ? "ok   " : "FAIL "} ${mode}: ${message}`);
  if (!condition) failures.push(`${mode}: ${message}`);
};
...
/** The public API from a page; `worker` puts the kernel in the package's worker. */
async function runSuite(worker) {
  mode = worker ? "worker" : "page";
  const query = worker ? "?worker=1" : "";
  const errors = [];
  const page = await browser.newPage({ viewport: { width: 1000, height: 700 } });
  page.on("pageerror", (error) => errors.push(error.message));
  page.on("console", (message) => { if (message.type() === "error") errors.push(message.text()); });
  try {
    await page.goto(`${server.origin}/examples/embed-viewer/${query}`);
    await page.waitForFunction(() => Boolean(window.tessifcViewer), null, { timeout: 60_000 });

    await page.evaluate(() => {
      const v = window.tessifcViewer;
      window.__events = { load: 0, progress: 0, close: 0, overlay: [], select: [], visibility: [], reopen: [] };
      window.__longTasks = [];
      ...
      v.on("reopen", (detail) => { window.__events.reopen.push(detail); });
      if (typeof PerformanceObserver === "function") {
        try {
          new PerformanceObserver((list) => { for (const entry of list.getEntries()) window.__longTasks.push(entry.duration); })
            .observe({ type: "longtask", buffered: true });
        } catch {
          // No long-task timing here; the worker check reports it as unmeasured.
        }
      }
    });
    ...
    const loaded = await page.evaluate(() => {
      ...
      return {
        ...
        worker: v.worker(),
        pageKernel: Boolean(window.tessifcKernel),
    ...
    if (worker) {
      check(loaded.worker?.running === true && typeof loaded.worker.version === "string" && !loaded.pageKernel,
        `the kernel runs in the package's worker and the page loaded none (${loaded.worker?.version})`);
      const longest = Math.max(0, ...loaded.longTasks);
      check(loaded.longTasks.length === 0 || longest < 220,
        `no page task during the open outlasts a chunk budget (${loaded.longTasks.length} long tasks, longest ${longest.toFixed(0)} ms)`);
    } else {
      check(loaded.worker === null && loaded.pageKernel, "the page-thread path reports no worker");
    }
    ...
    const raced = await page.evaluate(async () => {
      ...
      return { first: a, second: b !== null && v.modelId() === b.modelId, models: kernel ? kernel.modelCount() : 1, loads: window.__events.load - loads, closes: window.__events.close - closes };
    });
    ...
    if (worker) {
      // The session lives in the worker: its calls return promises, and its deltas feed the same applyDelta.
      const edited = await page.evaluate(async () => {
        const v = window.tessifcViewer;
        const revisions = [];
        const off = v.on("revision", (detail) => revisions.push(detail));
        const session = v.session();
        const remote = session.remote === true && typeof session.runScript === "function";
        const wallIds = await session.idsOfType("IfcWall");
        const info = await session.info();
        const { report, delta } = await session.runScript(`ifc.addWall({ from: [20, 20], to: [26, 20], height: 3, name: "Worker wall" }); print("done");`);
        const applied = v.applyDelta(delta);
        v.renderer.render(true);
        const entity = await session.entity(delta.affectedProducts[0]);
        const history = session.history;
        const undone = await session.undo();
        v.applyDelta(undone);
        off();
        return { remote, walls: wallIds.length, entities: info.entities, ok: report.ok, stdout: report.stdout.trim(), revision: delta.revision,
          affected: delta.affectedProducts.length, applied, entityClass: entity?.class, history, undoneRevision: undone.revision,
          sessionRevision: session.revision, events: revisions.map((r) => r.revision), instances: v.pack().instances.active.reduce((a, b) => a + b, 0) };
      });
      check(edited.remote && edited.walls > 0 && edited.entities > 0, `the worker session answers questions (${edited.walls} walls, ${edited.entities} entities)`);
      check(edited.ok && edited.stdout === "done" && edited.revision === "1" && edited.affected === 1 && edited.applied.changed && edited.entityClass === "IfcWall",
        "a script run in the worker publishes a delta the renderer applies and the entity can be read back");
      check(edited.history.undo === 1 && edited.undoneRevision === "2" && edited.sessionRevision === "2" && JSON.stringify(edited.events) === JSON.stringify(["1", "2"]),
        `undo publishes the previous content as a new revision and each delta emits a revision event (${edited.events})`);
    }

    const closed = await page.evaluate(() => {
      const v = window.tessifcViewer;
      v.close();
      const kernelEmpty = window.tessifcKernel ? window.tessifcKernel.modelCount() === 0 : true;
      const emptied = v.pack() === null && v.renderer.batches.length === 0 && kernelEmpty;
      const workerKept = v.worker()?.running ?? null;
      v.dispose();
      return { emptied, workerKept, workerGone: v.worker()?.running === false || v.worker() === null, canvasGone: !document.getElementById("host").querySelector("canvas") };
    });
    ...
    if (worker) check(closed.workerKept === true && closed.workerGone, "close keeps the worker for the next open; dispose ends it");

    // Without the worker the overlay event still settles, so a host waiting on it is not left hanging.
    const plain = await page.evaluate(async (inWorker) => {
      const { createViewer } = await import("../../bindings/viewer/src/index.js");
      const v = createViewer(document.getElementById("host"), inWorker ? { worker: true, coincidence: false } : { kernel: window.tessifcKernel, coincidence: false });
      const events = [];
      v.on("overlay", (detail) => events.push(detail));
      await v.open(document.getElementById("file").files[0]);
      const state = v.overlayState();
      v.dispose();
      return { state, events, models: window.tessifcKernel ? window.tessifcKernel.modelCount() : 0 };
    }, worker);
    ...
    const built = await page.evaluate(async (inWorker) => {
      const { createViewer } = await import("../../bindings/viewer/src/index.js");
      ...
      const session = v.session();
      const { report, delta } = await session.runScript('const w = ifc.addWall({ from: [0, 0], to: [4, 0], height: 3 }); ifc.addColumn({ at: [1, 1], height: 3 }); print(w.id);');
      const applied = v.applyDelta(delta);
      v.renderer.render(true);
      const wallId = Number(report.stdout);
      v.select(wallId);
      const second = await session.runScript(`ifc.get(${wallId}).Representation.Representations[0].Items[0].Depth = 4;`);
    ...
    if (worker) {
      // A script past its limit ends the worker; the model comes back from its last revision and stays drawn.
      await page.goto(`${server.origin}/examples/embed-viewer/?worker=1&scriptTimeoutMs=500`);
      await page.waitForFunction(() => Boolean(window.tessifcViewer), null, { timeout: 60_000 });
      await page.setInputFiles("#file", pavilionFile());
      await page.waitForFunction(() => window.tessifcViewer.pack() !== null && window.tessifcViewer.worker()?.running, null, { timeout: 120_000 });
      const stopped = await page.evaluate(async () => {
        const v = window.tessifcViewer;
        const reopens = [];
        v.on("reopen", (detail) => reopens.push(detail));
        const modelId = v.modelId();
        const instances = v.pack().instances.count;
        const session = v.session();
        const first = await session.runScript(`ifc.addWall({ from: [30, 30], to: [36, 30], height: 3 });`);
        v.applyDelta(first.delta);
        const started = performance.now();
        const looped = await session.runScript("while (true) {}");
        const elapsed = performance.now() - started;
        const after = await session.runScript('print(ifc.byType("IfcWall").length);');
        v.renderer.render(true);
        return { timedOut: looped.report?.timedOut === true && looped.report.ok === false && looped.delta === null, elapsed, reopens: reopens.length,
          reopenedId: reopens[0]?.modelId, newModelId: v.modelId(), hadModel: Number.isInteger(modelId), instances: v.pack().instances.active.reduce((a, b) => a + b, 0), before: instances,
          batches: v.renderer.batches.length, revision: session.revision, history: session.history, walls: Number(after.report?.stdout), afterOk: after.report?.ok === true,
          running: v.worker()?.running, scriptTimeoutMs: v.worker()?.scriptTimeoutMs };
      });
      check(stopped.scriptTimeoutMs === 500 && stopped.timedOut && stopped.elapsed < 10_000, `a looping script resolves timedOut after its limit (${stopped.elapsed.toFixed(0)} ms)`);
      // A fresh worker numbers its models from the start again, so only the event's id is compared.
      check(stopped.reopens === 1 && stopped.hadModel && stopped.reopenedId === stopped.newModelId && stopped.running === true,
        "the model is reopened in a fresh worker and the reopen event names the model id in view");
      check(stopped.instances === stopped.before + 1 && stopped.batches > 0, `the products stay drawn, the committed wall included (${stopped.instances} instances)`);
      check(stopped.revision === "0" && stopped.history.undo === 0 && stopped.afterOk && stopped.walls > 0,
        `the reopened model starts at revision 0 with no history and takes scripts again (${stopped.walls} walls)`);
    }
...
console.log("PASS  the embedded viewer package opens, selects, hides, sections and disposes, on the page and in a worker");
