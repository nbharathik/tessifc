...
pub struct ProductPart {
    ...
    /// The material it is drawn with, when the file styles it and textures are wanted.
    pub material: Option<Arc<Material>>,
}

/// Whether two parts may share a colour slot: materials that look the same,
/// whichever style entity each came from, or none.
    ...
    match (a, b) {
        ...
        (Some(a), Some(b)) => {
            a.colour == b.colour
                ...
                && a.texture.as_ref().map(|texture| texture.id)
                    == b.texture.as_ref().map(|texture| texture.id)
        }
        _ => false,
    }
}

/// Generate coordinates for a mesh whose texture asks for them and which has none.
fn apply_generator(
    ctx: &EvalCtx<'_>,
    mesh: &mut Mesh64,
    ...
    let Some(generator) = &texture.generator else {
        return;
    };
...
pub fn product_parts(
    ...
    for part in parts {
        ...
        let slot = match colours.iter().enumerate().position(|(index, known)| {
            *known == colour
                && same_material(&materials[index], &part.material)
        ...
        match result {
            ...
            Some(result) => {
                if body.has_uvs() && !result.has_uvs() {
                    ctx.diag.warn(
                        codes::TEXTURE_DROPPED_BY_BOOLEAN,
                        product.id(),
                        "cutting the body lost its texture coordinates; the texture is carried without them",
                    );
                }
                mesh.append(&result)
            }
...
/// The item's material, read only when textures are wanted.
fn material_of(ctx: &EvalCtx<'_>, item: Entity<'_>) -> Option<Arc<Material>> {
    if ctx.settings.textures {
        crate::style::item_material(ctx, item)
    } else {
        None
    }
}
    ...
    for value in items {
        ...
        if let Some(difference) = crate::eval::solids::evaluate_difference(ctx, registry, item) {
            for reason in &difference.refused {
                ctx.diag
                    .warn(codes::BOOLEAN_REFUSED, item.id(), reason.clone());
            }
            if !difference.body.is_empty() {
                let material = material_of(ctx, item);
                let mut body = difference.body;
                apply_generator(ctx, &mut body, &material, item.id());
                parts.push(BodyPart {
                    geometry: BodyGeometry::Unique(body),
            ...
            && let Some(groups) = crate::eval::tessellated::coloured_parts(ctx, item)
        {
            ...
            for (colour, mut mesh) in groups {
                apply_generator(ctx, &mut mesh, &material, item.id());
                parts.push(BodyPart {
        ...
        match registry.solid(ctx, item) {
            Ok(mut part) => {
                if !part.is_empty() {
                    let material = material_of(ctx, item);
                    apply_generator(ctx, &mut part, &material, item.id());
                    parts.push(BodyPart {
...
/// at its grid's origin. A linear placement is taken from its `CartesianPosition`;
/// without one it is reported and sits at its parent's origin, as does any
/// placement kind this kernel does not resolve.
pub fn product_transform(ctx: &EvalCtx<'_>, model: &Model, product: Entity<'_>) -> DMat4 {
    ...
    let resolver = |placement: Entity<'_>| {
        if placement.is_a("IfcGridPlacement") {
            return match crate::grid::grid_placement(ctx, model, placement) {
        ...
        if placement.is_a("IfcLinearPlacement") {
            return match crate::eval::alignment::linear_placement(ctx, placement, product.id()) {
                Ok(resolved) => Some(resolved),
                Err(error) => {
                    let fallback = if placement.attr("CartesianPosition").as_entity().is_some() {
                        "its CartesianPosition is used"
                    } else {
                        "the product sits at its parent's origin"
                    };
                    ctx.diag.warn(
                        codes::PLACEMENT_UNSUPPORTED,
                        product.id(),
                        format!("IfcLinearPlacement could not be resolved along its curve ({error}); {fallback}"),
                    );
                    None
                }
            };
        ...
        ctx.diag.warn(
            ...
            product.id(),
            format!(
                "{} is not a placement this kernel resolves; the product sits at its parent's origin",
...
mod tests {
    ...
    fn textured_parts(
        source: &str,
        product: u32,
    ) -> (Vec<ProductPart>, Vec<tessifc_step::Diagnostic>) {
        ...
        let parts = product_parts(&ctx, &registry, &model, model.entity(product).unwrap())
            .unwrap_or_default();
        (parts, sink.take())
    }

    /// The textured quad of the style tests as a wall's body.
    fn textured_wall() -> String {
        format!(
            "{}#20=IFCSHAPEREPRESENTATION($,'Body','Tessellation',(#2));\n\
             #21=IFCPRODUCTDEFINITIONSHAPE($,$,(#20));\n\
             #22=IFCWALL('w',$,$,$,$,$,#21,$,$);\n",
            crate::style::tests::TEXTURED_QUAD
        )
    }

    #[test]
    fn a_textured_product_carries_its_material_and_coordinates() {
        let (parts, diagnostics) = textured_parts(&textured_wall(), 22);
        assert!(diagnostics.is_empty(), "{diagnostics:?}");
        assert_eq!(parts.len(), 1);
        let material = parts[0].material.as_ref().expect("the material");
        assert_eq!(material.texture.as_ref().unwrap().id, 10);
        assert!(parts[0].geometry.local_mesh().has_uvs());
        assert_eq!(parts[0].color, Rgba([204, 51, 26, 255]));
    }

    #[test]
    fn textures_off_changes_nothing() {
        let (parts, _) = evaluate_parts(&textured_wall(), 22);
        assert_eq!(parts.len(), 1);
        assert!(parts[0].material.is_none());
        assert!(!parts[0].geometry.local_mesh().has_uvs());
        assert_eq!(parts[0].color, Rgba([204, 51, 26, 255]));
    }

    #[test]
    fn a_generator_projects_an_untextured_body_by_axis() {
        let source = format!(
            "{}#40=IFCTEXTURECOORDINATEGENERATOR((#10),'COORD',$);\n",
            wall_model("")
        ) + "#41=IFCIMAGETEXTURE(.T.,.T.,$,$,$,'a.png');\n\
             #42=IFCSURFACESTYLEWITHTEXTURES((#41));\n\
             #43=IFCCOLOURRGB($,0.5,0.5,0.5);\n\
             #44=IFCSURFACESTYLESHADING(#43);\n\
             #45=IFCSURFACESTYLE('s',.BOTH.,(#44,#42));\n\
             #46=IFCSTYLEDITEM(#3,(#45),$);\n\
             #47=IFCTEXTURECOORDINATEGENERATOR((#41),'COORD',$);\n";
        let (parts, diagnostics) = textured_parts(&source, 9);
        assert!(diagnostics.is_empty(), "{diagnostics:?}");
        let mesh = parts[0].geometry.local_mesh();
        assert!(mesh.has_uvs());
        // Coordinates are metres from the box corner: the 4 m height reaches 4.
        let widest = mesh.uvs.iter().map(|uv| uv[1]).fold(0.0f32, f32::max);
        assert!((widest - 4.0).abs() < 1e-6, "{widest}");
    }

    #[test]
    fn a_generator_in_another_mode_is_noted() {
        let source = wall_model("")
            + "#41=IFCIMAGETEXTURE(.T.,.T.,$,$,$,'a.png');\n\
             #42=IFCSURFACESTYLEWITHTEXTURES((#41));\n\
             #43=IFCCOLOURRGB($,0.5,0.5,0.5);\n\
             #44=IFCSURFACESTYLESHADING(#43);\n\
             #45=IFCSURFACESTYLE('s',.BOTH.,(#44,#42));\n\
             #46=IFCSTYLEDITEM(#3,(#45),$);\n\
             #47=IFCTEXTURECOORDINATEGENERATOR((#41),'SPHERE',$);\n";
        let (parts, diagnostics) = textured_parts(&source, 9);
        assert!(!parts[0].geometry.local_mesh().has_uvs());
        assert!(
            diagnostics
                .iter()
                .any(|d| d.code == codes::TEXTURE_GENERATOR_UNSUPPORTED),
            "{diagnostics:?}"
        );
    }

    #[test]
    fn a_cut_textured_part_loses_its_coordinates_and_says_so() {
        // The generated box, voided by an opening through its middle.
        let source = wall_model("")
            + "#41=IFCIMAGETEXTURE(.T.,.T.,$,$,$,'a.png');\n\
             #42=IFCSURFACESTYLEWITHTEXTURES((#41));\n\
             #43=IFCCOLOURRGB($,0.5,0.5,0.5);\n\
             ...
             #46=IFCSTYLEDITEM(#3,(#45),$);\n\
             #47=IFCTEXTURECOORDINATEGENERATOR((#41),'COORD',$);\n\
             #50=IFCRECTANGLEPROFILEDEF(.AREA.,$,$,0.5,0.5);\n\
             #51=IFCCARTESIANPOINT((0.,0.,1.));\n\
             #52=IFCAXIS2PLACEMENT3D(#51,$,$);\n\
             #53=IFCEXTRUDEDAREASOLID(#50,#52,#2,1.);\n\
             #54=IFCSHAPEREPRESENTATION($,'Body','SweptSolid',(#53));\n\
             #55=IFCPRODUCTDEFINITIONSHAPE($,$,(#54));\n\
             #56=IFCLOCALPLACEMENT(#8,#7);\n\
             #57=IFCOPENINGELEMENT('o',$,$,$,$,#56,#55,$,$);\n\
             #58=IFCRELVOIDSELEMENT('v',$,$,$,#9,#57);\n";
        let (parts, diagnostics) = textured_parts(&source, 9);
        assert!(
            diagnostics
                .iter()
                .any(|d| d.code == codes::TEXTURE_DROPPED_BY_BOOLEAN),
            "{diagnostics:?}"
        );
        assert!(!parts[0].geometry.local_mesh().has_uvs());
        assert!(parts[0].material.is_some(), "the material stays");
    }
    ...
    fn a_refused_difference_keeps_its_provenance() {
        ...
        assert!(diagnostics.iter().any(|d| d.code == codes::BOOLEAN_REFUSED));
    }
    ...
    fn says_the_chain_is_cyclic(diagnostics: &[tessifc_step::Diagnostic]) -> bool {
        diagnostics
            .iter()
            .any(|d| d.code == codes::BOOLEAN_REFUSED && d.message.contains("cycle"))
    }
...
";
        ...
        crate::eval::tests::model_of_schema(
            ...
            &format!(
                "#1=IFCCARTESIANPOINT((0.,0.,0.));\n\
                 ...
                 #3=IFCVECTOR(#2,1.);\n\
                 #4=IFCLINE(#1,#3);\n\
                 #5=IFCPOINTBYDISTANCEEXPRESSION(IFCLENGTHMEASURE(3.),$,$,$,#4);\n\
                 ...
                 #12=IFCLINEARPLACEMENT(#9,#6,{cartesian_position});\n\
                 #13=IFCWALL('guid',$,'W',$,$,#12,$,$,$);\n"
            ),
        )
    }

    fn transform_with_diagnostics(
        model: &tessifc_model::Model,
        id: u32,
    ) -> (DMat4, Vec<tessifc_step::Diagnostic>) {
        let settings = Settings::default();
        let sink = DiagnosticSink::default();
        let ctx = EvalCtx::new(
            model,
        ...
        let world = product_transform(&ctx, model, model.entity(id).unwrap());
        (world, sink.take())
    }
    ...
    fn a_linear_placement_with_cartesian_position_uses_it() {
        ...
        let (world, diagnostics) = transform_with_diagnostics(&model, 13);
        assert!(diagnostics.is_empty(), "{diagnostics:?}");
        let origin = world.transform_point3(DVec3::ZERO);
        assert!(
            (origin - DVec3::new(13.0, 0.0, 0.0)).length() < 1e-9,
            "three along the curve, relative to the parent at ten: got {origin}"
        );
    }
    ...
    fn a_linear_placement_without_cartesian_position_is_resolved_along_its_curve() {
        ...
        assert!(
            (origin - DVec3::new(13.0, 0.0, 0.0)).length() < 1e-9,
            "three along the line, relative to the parent at ten: got {origin}"
        );
    }
    ...
    fn a_cached_position_that_disagrees_with_the_curve_is_reported() {
        ...
        let model = crate::eval::tests::model_of_schema(
            ...
            "#1=IFCCARTESIANPOINT((0.,0.,0.));\n\
             ...
             #11=IFCAXIS2PLACEMENT3D(#10,$,$);\n\
             #12=IFCLINEARPLACEMENT($,#6,#11);\n\
             #13=IFCWALL('guid',$,'W',$,$,#12,$,$,$);\n",
        );
        let (world, diagnostics) = transform_with_diagnostics(&model, 13);
        assert!(
            diagnostics.iter().any(|item| {
                item.code == codes::LINEAR_PLACEMENT_MISMATCH && item.express_id == Some(13)
            }),
            "{diagnostics:?}"
        );
        let origin = world.transform_point3(DVec3::ZERO);
        assert!(
            (origin - DVec3::new(3.0, 0.0, 0.0)).length() < 1e-9,
            "got {origin}"
        );
    }
    ...
    fn a_linear_placement_on_an_unreadable_curve_warns_and_uses_its_cache() {
        ...
        let model = crate::eval::tests::model_of_schema(
            "IFC4X3_ADD2",
            "#5=IFCPOINTBYDISTANCEEXPRESSION(IFCLENGTHMEASURE(3.),$,$,$,$);\n\
             #6=IFCAXIS2PLACEMENTLINEAR(#5,$,$);\n\
             #10=IFCCARTESIANPOINT((1.,0.,0.));\n\
             #11=IFCAXIS2PLACEMENT3D(#10,$,$);\n\
             #12=IFCLINEARPLACEMENT($,#6,#11);\n\
        ...
        assert!(
            diagnostics.iter().any(|item| {
                item.code == codes::PLACEMENT_UNSUPPORTED && item.express_id == Some(13)
            }),
            "{diagnostics:?}"
        );
        let origin = world.transform_point3(DVec3::ZERO);
        assert!(
            (origin - DVec3::new(1.0, 0.0, 0.0)).length() < 1e-9,
            "got {origin}"
        );
    }

    #[test]
    fn an_unknown_placement_kind_warns_and_keeps_its_parent() {
        // A local placement whose parent is not a placement class at all.
        let model = model_of(concat!(
            "#1=IFCCARTESIANPOINT((10.,0.,0.));\n",
            "#2=IFCAXIS2PLACEMENT3D(#1,$,$);\n",
            "#3=IFCLOCALPLACEMENT(#2,#2);\n",
            "#4=IFCWALL('guid',$,'W',$,$,#3,$,$,$);\n",
        ));
        let (world, diagnostics) = transform_with_diagnostics(&model, 4);
        assert!(
            diagnostics.iter().any(|item| {
                item.code == codes::PLACEMENT_UNSUPPORTED
                    && item.message.contains("IfcAxis2Placement3D")
            }),
            "{diagnostics:?}"
        );
        let origin = world.transform_point3(DVec3::ZERO);
        assert!(
            (origin - DVec3::new(10.0, 0.0, 0.0)).length() < 1e-9,
            "got {origin}"
        );
    }
}
