...
TessIFC 0.2.0 provides a Rust geometry kernel, CLI, browser and Node WASM
bindings, an editing session with scripts and incremental revisions, an
embeddable viewer package, a three.js adapter, a local browser viewer, an MCP
server for agents and an optional Python session over IfcOpenShell with the
same agent tools. This release is intended for integration feedback and
geometry evaluation. It is not a claim of complete IFC conformance, universal
...
* Scripts, whether typed, generated by an assistant or sent by an agent over
  MCP, run without a sandbox and with the host's permissions. The MCP and
  session servers bind to the loopback interface and expect one user.
